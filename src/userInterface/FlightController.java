    ...
    */
    ...
    public void generateFlights(ActionEvent event) {
    	...
    	for (int i = 0; i < numberFlight ; i++) {
    		Flight f = new Flight(fligths.getRandomDate(),fligths.getRandomTime(), fligths.getRandomAirline(), fligths.getRandomFlight(), fligths.getRandomDestination(), fligths.randomGate());
    		flight.add(f);
		}
    	showTable();
    }
    ...
    public void showTable() {
    	ArrayList<Flight> flight = fligths.getFlight();
    	int pages = flight.size()/18;
    	
    	for(int j=0; j<=pages; j++) {
    		if(j+1 == Integer.parseInt(page.getText())) {	
    			for (int i = 18*j; i < 18+(18*j) && i<flight.size(); i++) {
    					Label timeF = new Label("\t"+flight.get(i).getTime().toString());
    					Label dateF = new Label("\t"+flight.get(i).getDate().toString());
    					Label airlineF = new Label("\t"+flight.get(i).getAirline());
    					Label numF = new Label("\t"+flight.get(i).getNumFlight());
    					Label destinationF = new Label("\t"+flight.get(i).getDestination());
    					Label gateF = new Label("\t"+flight.get(i).getGate());			
	 ...
	 * Descripción:Mediante el evento que se produce en el botón DATE en la Interfaz,se muestra la 
	 * lista de vuelos organizados  invocando el metodo insertionSortTime que ordena mediante el 
	 * ALGORITMO DE INSERCION<br>
	 * @param event:ActionEvent
     * <b>post: </b>Genera la lista de vuelos ordenada por TIME 
